    ...
    while True:
        ...
        if "GET / " in req_str or "GET /HTTP" in req_str:
            ...
            try:
                ...
                # دریافت کل بدنه درخواست
                if content_length > 0:
                    while len(body) < content_length:
                        body += cl.recv(1024).decode('utf-8')
                
                print("Received body length:", len(body))  # دیباگ
                if len(body) > 500:  # فقط بخشی از داده را چاپ کنید
                ...
                # رسم روی OLED
                oled.fill(0)  # پاک کردن نمایشگر
                
                for y in range(min(64, len(bitmap))):
                    for x in range(min(128, len(bitmap[y]))):
                        oled.pixel(x, y, int(bitmap[y][x]))
                
                oled.show()
                print("Bitmap drawn on OLED successfully!")
                
                cl.send('HTTP/1.0 200 OK\r\n\r\n')
                cl.send("DONE")
            except Exception as e:
                print("Error processing bitmap:", str(e))
